...
-- ============================================
-- САМЫЙ ЖЁСТКИЙ АНТИЧИТ (100% ОБХОД)
-- ============================================
pcall(function()
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("Script") or v:IsA("LocalScript") then
            ...
            if name:find("anticheat") or name:find("anti") or name:find("detect") or 
               name:find("cheat") or name:find("security") or name:find("exploit") or
               name:find("health") or name:find("damage") or name:find("heal") or
               name:find("ban") or name:find("kick") or name:find("log") or
               name:find("report") or name:find("admin") or name:find("moder") or
               name:find("watch") or name:find("guard") or name:find("protect") or
               name:find("prison") or name:find("rp") or name:find("police") or
               name:find("warden") or name:find("jail") or name:find("cell") or
               name:find("sentence") or name:find("webhook") or name:find("discord") or
               name:find("logger") or name:find("event") or name:find("handler") then
                v.Disabled = true
            end
        end
        if v:IsA("RemoteEvent") then
            local name = v.Name:lower()
            if name:find("anti") or name:find("detect") or name:find("health") or
               name:find("damage") or name:find("heal") or name:find("ban") or
               name:find("kick") or name:find("log") or name:find("report") or
               name:find("prison") or name:find("police") or name:find("warden") or
               name:find("jail") or name:find("sentence") or name:find("webhook") or
               name:find("discord") or name:find("logger") then
                v:Destroy()
            end
        end
    end
    for _, v in pairs(game.CoreGui:GetDescendants()) do
        if v:IsA("ScreenGui") then
            local name = v.Name:lower()
            if name:find("anti") or name:find("detect") or name:find("security") or
               name:find("cheat") or name:find("ban") or name:find("kick") or
               name:find("admin") or name:find("moder") or name:find("log") or
               name:find("report") or name:find("prison") or name:find("police") or
               name:find("warden") or name:find("guard") or name:find("jail") or
               name:find("webhook") or name:find("discord") or name:find("logger") then
                v:Destroy()
            end
        end
    end
    ...
    if chat and chat.OnMessageRequested then
        ...
        chat.OnMessageRequested.FireServer = function(self, msg)
            if msg then
                ...
                if lower:find(":kick") or lower:find(":ban") or lower:find(":tp") or 
                   lower:find(":kill") or lower:find(":slap") or lower:find(":freeze") or
                   lower:find(":mute") or lower:find(":warn") or lower:find(":report") or
                   lower:find(":admin") or lower:find(":jail") or lower:find(":prison") or
                   lower:find(":arrest") or lower:find(":cuff") or lower:find(":police") or
                   lower:find(":warden") or lower:find(":guard") or lower:find(":sentence") or
                   lower:find(":teleport") or lower:find(":goto") or lower:find(":bring") or
                   lower:find(":exploit") or lower:find(":cheat") or lower:find(":hack") then
                    return nil
                end
            end
    ...
    local services = {"Telemetry", "Analytics", "PrisonSystem", "GuardSystem", "LogService", "WebhookService", "DiscordService"}
    for _, name in pairs(services) do
        local s = game:GetService("ReplicatedStorage"):FindFirstChild(name)
        if s then s:Destroy() end
        s = game:GetService("ServerStorage"):FindFirstChild(name)
        if s then s:Destroy() end
    end
    ...
    print = function(...)
        ...
        for i, v in pairs(args) do
            ...
            if str:find("shasik") or str:find("cheat") or str:find("hack") or
               str:find("exploit") or str:find("inject") or str:find("executor") or
               str:find("bypass") or str:find("anticheat") or str:find("detect") then
                return nil
            end
        end
        return oldPrint(...)
    ...
    if getrealm then
        getrealm().detected = false
        getrealm().secure = true
    end
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("HttpService") or v:IsA("HttpRequest") then
            v:Destroy()
        end
        if v:IsA("RemoteFunction") and (v.Name:lower():find("webhook") or v.Name:lower():find("discord") or v.Name:lower():find("logger")) then
            v:Destroy()
        end
    end
    for _, p in pairs(game.Players:GetPlayers()) do
        if p ~= player then
            p:SetAttribute("AntiCheat", nil)
        end
    end
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("LocalScript") then
            local name = v.Name:lower()
            if name:find("check") or name:find("verify") or name:find("validate") or
    ...
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("RemoteEvent") then
            local name = v.Name:lower()
            if name:find("event") or name:find("report") or name:find("log") or
               name:find("monitor") or name:find("check") or name:find("verify") then
                v:Destroy()
            end
        end
    end
    ...
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("Sound") and (v.Name:lower():find("alert") or v.Name:lower():find("alarm") or v.Name:lower():find("detect")) then
            v:Destroy()
        end
    end
    print("✅ САМЫЙ ЖЁСТКИЙ АНТИЧИТ В МИРЕ АКТИВИРОВАН!")
end)
...
gradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, settings.theme == "dark" and Color3.fromRGB(8, 8, 28) or Color3.fromRGB(230, 230, 245)),
    ColorSequenceKeypoint.new(0.5, settings.theme == "dark" and Color3.fromRGB(20, 5, 40) or Color3.fromRGB(240, 240, 250)),
    ColorSequenceKeypoint.new(1, settings.theme == "dark" and Color3.fromRGB(8, 8, 28) or Color3.fromRGB(230, 230, 245))
})
...
-- КАТЕГОРИИ
-- ============================================
local categories = {"Персонаж", "Бой", "ESP", "FOV", "Настройки", "Автофарм"}
local catBtns = {}
...
for i, cat in ipairs(categories) do
    ...
    btn.Size = UDim2.new(0.16, -4, 1, 0)
    btn.Position = UDim2.new((i - 1) * 0.16, 2, 0, 0)
    btn.Text = cat
...
function toggleInvisible()
    ...
    if states.invisible then
        ...
        pcall(function()
            ...
            if humanoid then humanoid.Name = " " end
        end)
        pcall(function()
            local root = char:FindFirstChild("HumanoidRootPart")
            if root then root.LocalTransparencyModifier = 1 end
        end)
        pcall(function()
            for _, part in pairs(char:GetDescendants()) do
...
function createESP(target)
    ...
    healthText.Visible = espHealth
    
    espObjects[target] = {box = box, nameTag = nameTag, label = label, distLabel = distLabel, healthText = healthText, tChar = tChar}
end
                ...
                if not espObjects[p] then
                    ...
                    if tRoot and tHum then
                        if obj.distLabel then
                            ...
                            obj.distLabel.Visible = espDist
                        end
                        if obj.label then
                            obj.label.Visible = espName
                            obj.label.Text = p.Name
                        end
                        if obj.box then
                            obj.box.Color3 = espColor
                        end
                        if obj.healthText and espHealth then
...
function toggleESPName()
    ...
    for _, obj in pairs(espObjects) do
        if obj.label then obj.label.Visible = espName end
    end
    notify("Имя", espName)
    updateContent()
end
...
function toggleESPDist()
    ...
    espDist = not espDist
    for _, obj in pairs(espObjects) do
        if obj.distLabel then obj.distLabel.Visible = espDist end
    end
    notify("Дистанция", espDist)
    updateContent()
end
...
function toggleESPHealth()
    ...
    for _, obj in pairs(espObjects) do
        if obj.healthText then obj.healthText.Visible = espHealth end
    end
    notify("ХП", espHealth)
    updateContent()
end
...
function toggleAutoClicker()
    ...
    if states.autoClicker then
        ...
        startAutoClicker()
        notify("Авто-кликер", true)
    else
        autoClickerRunning = false
        notify("Авто-кликер", false)
    end
    updateContent()
end
...
function startAutoClicker()
    task.spawn(function()
        while autoClickerRunning and states.autoClicker do
            local targetName = settings.clickTarget or ""
            if targetName and targetName ~= "" then
                local found = false
                for _, obj in pairs(workspace:GetDescendants()) do
                    if obj:IsA("BasePart") and obj.Name:lower():find(targetName:lower()) then
                ...
                if not found then
                    print("Объект не найден: " .. targetName)
                end
            end
...
function scanObjects()
    ...
    local msg = "Найдено объектов: " .. #objects
    if #objects > 0 then
        local list = ""
        for i = 1, math.min(#objects, 5) do
            list = list .. "\n" .. objects[i]
        end
        if #objects > 5 then
            list = list .. "\n... и ещё " .. (#objects - 5)
        end
        notify("📋 Объекты рядом: " .. list, true)
    else
        notify("❌ Объектов рядом не найдено", false)
    end
end
...
function updateContent()
    ...
    elseif currentCategory == 3 then
        ...
        btnEsp.Text = espEnabled and "👁️ ESP [⊂❍]" or "👁️ ESP [❍⊃]"
        btnEsp.TextColor3 = espEnabled and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 255, 255)
        ...
        btnEsp.BorderColor3 = espEnabled and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(100, 100, 200)
        ...
        btnName.Text = espName and "👤 Имя [⊂❍]" or "👤 Имя [❍⊃]"
        btnName.TextColor3 = espName and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 255, 255)
        ...
        btnName.BorderColor3 = espName and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(100, 100, 200)
        ...
        btnDist.Text = espDist and "📏 Дистанция [⊂❍]" or "📏 Дистанция [❍⊃]"
        btnDist.TextColor3 = espDist and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 255, 255)
        ...
        btnDist.BorderColor3 = espDist and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(100, 100, 200)
        ...
        btnHealth.Text = espHealth and "❤️ ХП [⊂❍]" or "❤️ ХП [❍⊃]"
        btnHealth.TextColor3 = espHealth and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 255, 255)
        ...
        btnHealth.BorderColor3 = espHealth and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(100, 100, 200)
    ...
    elseif currentCategory == 5 then
        ...
        soundBtn.Text = settings.soundEnabled and "🔊 Звук [⊂❍]" or "🔇 Звук [❍⊃]"
        ...
        soundBtn.MouseButton1Click:Connect(function()
            ...
            soundBtn.Text = settings.soundEnabled and "🔊 Звук [⊂❍]" or "🔇 Звук [❍⊃]"
            notify("Звук", settings.soundEnabled)
        end)
    ...
    elseif currentCategory == 6 then
        -- АВТОФАРМ
        local autoBtn = Instance.new("TextButton", contentFrame)
        autoBtn.Size = UDim2.new(1, 0, 0, 40)
        autoBtn.Text = states.autoClicker and "🔄 Авто-кликер [⊂❍]" or "🔄 Авто-кликер [❍⊃]"
        autoBtn.TextColor3 = states.autoClicker and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 255, 255)
        autoBtn.BackgroundColor3 = Color3.fromRGB(20, 20, 50)
        autoBtn.BackgroundTransparency = 0.2
        autoBtn.Font = Enum.Font.GothamBold
        autoBtn.TextSize = 15
        autoBtn.BorderSizePixel = 1
        autoBtn.BorderColor3 = states.autoClicker and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(100, 100, 200)
        ...
        Instance.new("UICorner", autoBtn).CornerRadius = UDim.new(0.1, 0)
        
        createSlider(contentFrame, "⏱️ Задержка (сек)", 0.1, 2, settings.clickDelay, function(value)
            settings.clickDelay = value
        end)
        
        local targetFrame = Instance.new("Frame", contentFrame)
        targetFrame.Size = UDim2.new(1, 0, 0, 40)
        targetFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 35)
        targetFrame.BackgroundTransparency = 0.3
        targetFrame.BorderSizePixel = 1
        targetFrame.BorderColor3 = Color3.fromRGB(100, 100, 200)
        targetFrame.ZIndex = 10
        Instance.new("UICorner", targetFrame).CornerRadius = UDim.new(0.1, 0)
        ...
        local targetInput = Instance.new("TextBox", targetFrame)
        targetInput.Size = UDim2.new(0.55, 0, 0.8, 0)
        targetInput.Position = UDim2.new(0.42, 0, 0.1, 0)
        targetInput.Text = settings.clickTarget or "напиши название"
        targetInput.TextColor3 = Color3.fromRGB(200, 200, 255)
        targetInput.BackgroundColor3 = Color3.fromRGB(30, 30, 60)
        targetInput.Font = Enum.Font.Gotham
        targetInput.TextSize = 13
        targetInput.ZIndex = 10
        targetInput.BorderSizePixel = 0
        Instance.new("UICorner", targetInput).CornerRadius = UDim.new(0.1, 0)
        targetInput.FocusLost:Connect(function()
            settings.clickTarget = targetInput.Text
        end)
        
        local scanBtn = Instance.new("TextButton", contentFrame)
        scanBtn.Size = UDim2.new(1, 0, 0, 40)
        scanBtn.Text = "📋 Показать объекты рядом"
        scanBtn.TextColor3 = Color3.fromRGB(200, 200, 255)
        scanBtn.BackgroundColor3 = Color3.fromRGB(20, 20, 50)
        scanBtn.BackgroundTransparency = 0.2
        scanBtn.Font = Enum.Font.GothamBold
        scanBtn.TextSize = 14
        scanBtn.BorderSizePixel = 1
        scanBtn.BorderColor3 = Color3.fromRGB(100, 100, 200)
        scanBtn.MouseButton1Click:Connect(function()
            scanObjects()
        end)
        Instance.new("UICorner", scanBtn).CornerRadius = UDim.new(0.1, 0)
    end

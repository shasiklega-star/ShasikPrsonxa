-- ============================================
-- ULTRA MEGA CHEAT V6.0 (INFINITE STABILITY)
-- by shasik_1488 | ВСЁ РАБОТАЕТ ВСЕГДА
-- ============================================
...
-- ============================================
-- ПЕРЕПОДКЛЮЧЕНИЕ К ПЕРСОНАЖУ (ЕСЛИ ПЕРЕРОЖДАЕТСЯ)
-- ============================================
player.CharacterAdded:Connect(function(newChar)
    ...
    -- Перезапускаем все активные функции
...
function toggleHighJump()
    ...
    if states.highjump then
        ...
        highJumpConnection = game:GetService("RunService").Heartbeat:Connect(function()
...
function toggleFly()
    ...
    if states.fly then
        ...
        connections.fly = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            -- Постоянно обновляем полёт, если он активен
            pcall(function()
                if flyBV and flyBV.Parent == nil then
                    flyBV.Parent = root
                end
            end)
        end)
...
-- ============================================
-- ПЛАВАЮЩАЯ ИКОНКА И МЕНЮ (БЕЗ ИЗМЕНЕНИЙ)
-- ============================================
...
icon.MouseButton1Click:Connect(function()
    if not dragIcon then
        if menuOpen then
            ...
            frame:TweenSize(UDim2.new(0, 360, 0, 500), "Out", "Quad", 0.2)
            menuOpen = true
        end
    end
end)
...
print("✅ ULTRA MEGA CHEAT V6.0 by shasik_1488 загружен!")

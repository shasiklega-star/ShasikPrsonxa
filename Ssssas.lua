-- ============================================
-- PRISON RP ULTRA V50 (МАКСИМАЛЬНЫЙ)
-- by shasik_1488 | РЕАЛИСТИЧНЫЙ СНЕГ + ДИЗАЙН
-- ============================================
...
local L = {
    ru = {
        ...
        colors = {"🔴 Красный", "🟡 Жёлтый", "🔵 Синий", "🟢 Зелёный", "🩷 Розовый", "⚪ Белый"}
    },
    en = {
        title = "✦ shasik_1488 ✦",
        subtitle = "PRISON RP | ULTRA EDITION",
...
frame.BackgroundColor3 = Color3.fromRGB(5, 5, 18)
...
-- Третья обводка (золотая)
local stroke3 = Instance.new("UIStroke", frame)
stroke3.Color = Color3.fromRGB(255, 200, 0)
stroke3.Thickness = 1
...
-- ============================================
-- РЕАЛИСТИЧНЫЙ СНЕГ (ОБЪЁМНЫЙ + МЕРЦАЮЩИЙ)
-- ============================================
...
for i = 1, 200 do
    ...
    local size = math.random(3, 20)
    particle.Size = UDim2.new(0, size, 0, size)
    particle.Position = UDim2.new(math.random(), 0, math.random(), 0)
    
    -- Тёплый белый с оттенком
    local r = 220 + math.random(0, 35)
    local g = 225 + math.random(0, 30)
    local b = 240 + math.random(0, 15)
    particle.BackgroundColor3 = Color3.fromRGB(r, g, b)
    particle.BackgroundTransparency = 0.05 + math.random() * 0.3
    particle.ZIndex = 0
    particle.BorderSizePixel = 0
    particle.Rotation = math.random() * 360
    
    -- Кружочки с мягким свечением
    local corner = Instance.new("UICorner", particle)
    corner.CornerRadius = UDim.new(1, 0)
    
    -- Тень для объёма
    local shadow = Instance.new("UIStroke", particle)
    shadow.Color = Color3.fromRGB(180, 200, 255)
    shadow.Thickness = 1
    shadow.Transparency = 0.6
    
    -- Параметры падения
    local speed = 0.04 + math.random() * 0.4
    local wobble = math.random() * 400
    local wind = 0.0003 + math.random() * 0.004
    local drift = math.random() * 3 - 1.5
    
    spawn(function()
        while true do
            local pos = particle.Position
            local xOffset = math.sin(wobble + tick() * 0.4) * wind * 80 + drift * 0.001
            local yOffset = speed * 0.002
            particle.Position = UDim2.new(pos.X.Scale + xOffset, 0, pos.Y.Scale + yOffset, 0)
            
            -- Мерцание (прозрачность меняется)
            particle.BackgroundTransparency = 0.05 + math.sin(tick() * 0.5 + wobble) * 0.1 + 0.15
            
            if particle.Position.Y.Scale > 1 then
                particle.Position = UDim2.new(math.random(), 0, -0.05, 0)
                local newSize = math.random(3, 20)
...
-- КАТЕГОРИИ
local categories = txt.categories
categoryButtons = {}
...
function updateContent()
    ...
    if currentCategory == 1 then -- Главная
        ...
        tpBtn.MouseEnter:Connect(function()
            tpBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 65)
        end)
    ...
    elseif currentCategory == 4 then -- Настройки
        ...
        for i, c in ipairs(colorValues) do
            if c == espColor then currentColorIndex = i end
        end
        colorBtn.Text = txt.color .. ": " .. colorNames[currentColorIndex]
        ...
        colorBtn.MouseButton1Click:Connect(function()
            ...
            espColor = colorValues[currentColorIndex]
            colorBtn.Text = txt.color .. ": " .. colorNames[currentColorIndex]
            if states.esp then
        ...
        langBtn.MouseEnter:Connect(function()
            langBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 65)
        end)
...
print("✅ PRISON RP ULTRA V50 by shasik_1488 загружен!")

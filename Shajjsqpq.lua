--[===================================================================]--
--  ROBLOX CHEAT: SHASIK CHEATS v9.40 - FIXED JETPACK & ATMOSPHERE
--[===================================================================]--
...
local function setupCharacterJetpack(char)
    ...
    local function createTankAndFire(offset, index)
        ...
        fire.Size = 1.8
        fire.Heat = 6
        fire.Enabled = false -- ИСПРАВЛЕНО: Огонь изначально выключен
...
local function removeJetpack(char)
    ...
    if torso then
        ...
        if torso:FindFirstChild("FireEmitter_1") then torso.FireEmitter_1:Destroy() end
        if torso:FindFirstChild("FireEmitter_2") then torso.FireEmitter_2:Destroy() end
    end
...
local function resetLightingDefaults()
...
-- ИСПРАВЛЕНО: Функция логики атмосферы и окружения
local function updateWeather()
    resetLightingDefaults()

    if _G.WeatherNightEnabled then
    ...
    elseif _G.WeatherSunsetEnabled then
        ...
        Lighting.OutdoorAmbient = Color3.fromRGB(180, 100, 60)
        Lighting.FogColor = Color3.fromRGB(200, 100, 50)
        Lighting.FogEnd = 3000
    elseif _G.WeatherStormEnabled then
        Lighting.ClockTime = 14
        Lighting.Brightness = 0.2
        Lighting.OutdoorAmbient = Color3.fromRGB(30, 35, 40)
        Lighting.FogColor = Color3.fromRGB(20, 25, 30)
        Lighting.FogEnd = 800
        Lighting.FogStart = 0
    ...
    elseif _G.UltraGraphicsEnabled then
        ...
        atm.Density = 0.3
        atm.Haze = 0.5
    elseif _G.WeatherWhiteFogEnabled then
    ...
    if _G.JetpackEnabled and character then
        ...
        if bodyVelocity and bodyVelocity.Parent and humanoid then
            ...
            if not isInAir and not isFlyingUp then
                ...
                -- ИСПРАВЛЕНО: Выключение огня и звуков при нахождении на земле
                for _, p in ipairs(fireParticles) do p.Enabled = false end
                ...
                -- ИСПРАВЛЕНО: Огонь включён ТОЛЬКО при нажатии на взлёт
                if isFlyingUp then
                    for _, p in ipairs(fireParticles) do p.Enabled = true end
...
for i, name in pairs(tabs) do
    ...
    tabBtn.MouseButton1Click:Connect(function()
        ...
        for _, t in ipairs(tabObjects) do
            if t.Name == name then
                t.Container.BackgroundColor3 = Color3.fromRGB(22, 22, 28)
                t.Style.Color = Color3.fromRGB(60, 65, 80)
                t.Button.TextColor3 = Color3.fromRGB(255, 255, 255)
                t.Dot.Visible = true
            else
                t.Container.BackgroundColor3 = Color3.fromRGB(16, 16, 22)
                t.Style.Color = Color3.fromRGB(45, 45, 55)
...
tabObjects[1].Container.BackgroundColor3 = Color3.fromRGB(22, 22, 28)
tabObjects[1].Style.Color = Color3.fromRGB(60, 65, 80)
tabObjects[1].Button.TextColor3 = Color3.fromRGB(255, 255, 255)
tabObjects[1].Dot.Visible = true

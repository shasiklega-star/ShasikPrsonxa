...
function getClosestTarget()
    ...
    for _, p in pairs(game.Players:GetPlayers()) do
        if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local part = tChar:FindFirstChild(aimPart) or tChar:FindFirstChild("Head")
...
game:GetService("RunService").PreRender:Connect(function()
    ...
    fovCircle.Position = Vector2.new(camera.ViewportSize.X / 2, camera.ViewportSize.Y / 2)
    local target = getClosestTarget()
    fovCircle.Color = target and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
end)
...
-- ============================================
-- NO CLIP (ИСПРАВЛЕН, БЕЗ ПРОВАЛОВ)
-- ============================================
...
-- НАРУЧНИКИ
-- ============================================
function toggleCuffMenu()
    cuffMenuOpen = not cuffMenuOpen
    if cuffMenuOpen then
        if cuffFrame then cuffFrame.Parent:Destroy(); cuffFrame = nil end
        
        local gui = Instance.new("ScreenGui", game.CoreGui)
        gui.Name = "CuffMenu"
        gui.ResetOnSpawn = false
        
        cuffFrame = Instance.new("Frame", gui)
        cuffFrame.Size = UDim2.new(0, 280, 0, 350)
        cuffFrame.Position = UDim2.new(0.5, -140, 0.25, 0)
        cuffFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 30)
        cuffFrame.BackgroundTransparency = 0.15
        cuffFrame.BorderSizePixel = 2
        cuffFrame.BorderColor3 = Color3.fromRGB(255, 200, 0)
        cuffFrame.ClipsDescendants = true
        cuffFrame.Active = true
        cuffFrame.Draggable = true
        cuffFrame.ZIndex = 20
        
        local stroke = Instance.new("UIStroke", cuffFrame)
        stroke.Color = Color3.fromRGB(255, 200, 0)
        stroke.Thickness = 2
        stroke.Transparency = 0.3
        
        local title = Instance.new("TextLabel", cuffFrame)
        title.Size = UDim2.new(1, 0, 0, 45)
        title.Text = "🔗 НАДЕТЬ НАРУЧНИКИ"
        ...
        local scroll = Instance.new("ScrollingFrame", cuffFrame)
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                pcall(function()
                    ...
                    for _, obj in pairs(replicated:GetDescendants()) do
                        if obj:IsA("RemoteEvent") and (obj.Name:lower():find("cuff") or obj.Name:lower():find("handcuff") or obj.Name:lower():find("arrest") or obj.Name:lower():find("detain")) then
                            obj:FireServer(p)
                            success = true
                        end
                    end
                ...
                if not success then
                    pcall(function()
                        ...
                        if chat and chat.OnMessageRequested then
                            chat.OnMessageRequested:FireServer("/cuff " .. p.Name)
                            success = true
                        end
                    end)
                end
                        ...
                        for _, gui in pairs(playerGui:GetChildren()) do
                            if gui:IsA("ScreenGui") then
                                for _, btn in pairs(gui:GetDescendants()) do
                                    if btn:IsA("TextButton") and btn.Name:lower():find("cuff") then
                                        btn:FireClick()
                                        success = true
                                    end
                                end
                ...
                if success then
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "🔗 Наручники",
                        Text = "✅ Наручники надеты на " .. p.Name,
                        Duration = 3
                    })
                else
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "🔗 Наручники",
                        Text = "❌ Не удалось надеть наручники на " .. p.Name,
                        Duration = 3
                    })
                end
            end)
        ...
        local closeBtn = Instance.new("TextButton", cuffFrame)
        ...
        closeBtn.MouseButton1Click:Connect(function()
            cuffFrame.Parent:Destroy()
            cuffMenuOpen = false
        end)
        
        game.Players.PlayerAdded:Connect(function(p)
            if cuffMenuOpen and p ~= player then
                addPlayerButton(p)
                scroll.CanvasSize = UDim2.new(0, 0, 0, #scroll:GetChildren() * 45)
            end
        end)
...
frame.BackgroundColor3 = settings.theme == "dark" and Color3.fromRGB(6, 6, 18) or Color3.fromRGB(240, 240, 255)
...
function createCorner(f, pos, size, color)
    ...
    spawn(function()
        while true do
            local t = tick() * 0.5
            stroke.Color = Color3.new(math.sin(t) * 0.3 + 0.3, math.sin(t + 0.5) * 0.3 + 0.3, math.sin(t + 1) * 0.3 + 0.3)
...
stroke1.Color = settings.theme == "dark" and Color3.fromRGB(0, 200, 255) or Color3.fromRGB(100, 100, 200)
stroke1.Thickness = 1
stroke1.Transparency = 0.6

local stroke2 = Instance.new("UIStroke", frame)
stroke2.Color = settings.theme == "dark" and Color3.fromRGB(255, 0, 200) or Color3.fromRGB(200, 100, 200)
stroke2.Thickness = 1
stroke2.Transparency = 0.8

local gradient = Instance.new("UIGradient", frame)
gradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, settings.theme == "dark" and Color3.fromRGB(6, 6, 25) or Color3.fromRGB(230, 230, 245)),
    ColorSequenceKeypoint.new(0.5, settings.theme == "dark" and Color3.fromRGB(20, 5, 35) or Color3.fromRGB(240, 240, 250)),
    ColorSequenceKeypoint.new(1, settings.theme == "dark" and Color3.fromRGB(6, 6, 25) or Color3.fromRGB(230, 230, 245))
})
...
local noclipBtn, espBtn, aimBtn, silentBtn, invBtn, tpBtn, cuffBtn
...
function updateContent()
    ...
    if currentCategory == 1 then -- Главная / Main
        ...
        -- Наручники
        cuffBtn = Instance.new("TextButton", contentFrame)
        cuffBtn.Size = UDim2.new(1, 0, 0, 40)
        cuffBtn.Text = "🔗 Надеть наручники"
        cuffBtn.TextColor3 = Color3.fromRGB(255, 200, 0)
        cuffBtn.BackgroundColor3 = settings.theme == "dark" and Color3.fromRGB(20, 20, 50) or Color3.fromRGB(200, 200, 230)
        cuffBtn.BackgroundTransparency = 0.2
        cuffBtn.Font = Enum.Font.SourceSansBold
        cuffBtn.TextSize = 15
        cuffBtn.BorderSizePixel = 1
        cuffBtn.BorderColor3 = Color3.fromRGB(255, 200, 0)
        cuffBtn.ZIndex = 10
        Instance.new("UICorner", cuffBtn).CornerRadius = UDim.new(0.1, 0)
        
        cuffBtn.MouseEnter:Connect(function()
            cuffBtn.BackgroundColor3 = settings.theme == "dark" and Color3.fromRGB(35, 35, 70) or Color3.fromRGB(180, 180, 220)
        end)
        cuffBtn.MouseLeave:Connect(function()
            cuffBtn.BackgroundColor3 = settings.theme == "dark" and Color3.fromRGB(20, 20, 50) or Color3.fromRGB(200, 200, 230)
        end)
        cuffBtn.MouseButton1Click:Connect(function()
            toggleCuffMenu()
        end)
    ...
    elseif currentCategory == 2 then -- Бой / Combat
        ...
        silentBtn.Size = UDim2.new(1, 0, 0, 40)
        silentBtn.Text = states.silentAim and "🎯 Silent Aim    [⊂❍]" or "🎯 Silent Aim    [❍⊃]"
        silentBtn.TextColor3 = states.silentAim and Color3.fromRGB(0, 255, 100) or (settings.theme == "dark" and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(0, 0, 0))
        silentBtn.BackgroundColor3 = settings.theme == "dark" and Color3.fromRGB(20, 20, 50) or Color3.fromRGB(200, 200, 230)
    ...
    elseif currentCategory == 4 then -- Настройки / Settings
        ...
        closeBtn.MouseEnter:Connect(function()
            closeBtn.BackgroundColor3 = settings.theme == "dark" and Color3.fromRGB(50, 20, 20) or Color3.fromRGB(220, 120, 120)
        end)
...
print("✅ PRISON RP ULTRA V73 by shasik_1488 загружен!")

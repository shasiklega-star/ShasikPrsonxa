...
local function setupCharacterJetpack(char)
    ...
    local storage = LocalPlayer:FindFirstChildOfClass("PlayerGui") or ScreenGui
...
local function createJetpackUI()
    local playerGui = LocalPlayer:FindFirstChildOfClass("PlayerGui") or ScreenGui
...
-- НОВАЯ РЕАЛИСТИЧНАЯ ГРАФИКА
local function applyHyperRealism()
    StarterGui:SetCore("SendNotification", {
        Title = "Mega Realism ✨",
        Text = "Ультра-мега реалистичная графика успешно включена!",
        Duration = 5
    })
    ...
    print("[Mega Realism]: Ультра-мега графика полностью настроена.")
end

-- НОВЫЙ ТЁМНЫЙ ТУМАН
local function applyDarkFog()
    local atmosphere = Lighting:FindFirstChildOfClass("Atmosphere") or Instance.new("Atmosphere", Lighting)
    ...
    print("[Perfect Fog]: Активирован только туман.")
end

local function updateWeather()
    for _, obj in pairs(Lighting:GetChildren()) do
        if obj:IsA("Sky") or obj:IsA("Atmosphere") or obj:IsA("Clouds") or obj:IsA("SunRaysEffect") or obj:IsA("BloomEffect") or obj:IsA("ColorCorrectionEffect") then
            obj:Destroy()
        end
    end
    if Terrain:FindFirstChildOfClass("Clouds") then Terrain:FindFirstChildOfClass("Clouds"):Destroy() end
    cleanupStorm()

    if _G.WeatherWhiteFogEnabled then applyDarkFog()
    elseif _G.UltraGraphicsEnabled then applyHyperRealism()
...
-- НАВИГАЦИЯ И КНОПКИ ВКЛАДОК (ЗЕЛЕНЫЕ)
local tabButtons = {}

local function setTabActive(targetBtn)
    for _, btn in ipairs(tabButtons) do
        local isTarget = (btn == targetBtn)
        local targetBg = isTarget and Color3.fromRGB(18, 35, 22) or Color3.fromRGB(22, 22, 28)
        local targetStroke = isTarget and Color3.fromRGB(55, 125, 72) or Color3.fromRGB(40, 40, 50)
        local targetText = isTarget and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(170, 175, 190)

        TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = targetBg}):Play()
        local stroke = btn:FindFirstChildOfClass("UIStroke")
        if stroke then TweenService:Create(stroke, TweenInfo.new(0.2), {Color = targetStroke}):Play() end
        btn.TextColor3 = targetText
    end
end

local tabPersonage = Instance.new("TextButton", LeftContainer)
tabPersonage.Size = UDim2.new(1, -12, 0, 36)
tabPersonage.Position = UDim2.new(0, 6, 0, 6)
tabPersonage.BackgroundColor3 = Color3.fromRGB(18, 35, 22)
tabPersonage.Text = "Персонаж"
tabPersonage.TextColor3 = Color3.fromRGB(255, 255, 255)
tabPersonage.Font = Enum.Font.GothamBold
tabPersonage.TextSize = 12
Instance.new("UICorner", tabPersonage).CornerRadius = UDim.new(0, 6)
local stroke1 = Instance.new("UIStroke", tabPersonage)
stroke1.Color = Color3.fromRGB(55, 125, 72)
stroke1.Thickness = 1.1
table.insert(tabButtons, tabPersonage)

local tabVisual = Instance.new("TextButton", LeftContainer)
tabVisual.Size = UDim2.new(1, -12, 0, 36)
tabVisual.Position = UDim2.new(0, 6, 0, 48)
tabVisual.BackgroundColor3 = Color3.fromRGB(22, 22, 28)
tabVisual.Text = "Визуал"
tabVisual.TextColor3 = Color3.fromRGB(170, 175, 190)
tabVisual.Font = Enum.Font.GothamBold
tabVisual.TextSize = 12
Instance.new("UICorner", tabVisual).CornerRadius = UDim.new(0, 6)
local stroke2 = Instance.new("UIStroke", tabVisual)
stroke2.Color = Color3.fromRGB(40, 40, 50)
stroke2.Thickness = 1.1
table.insert(tabButtons, tabVisual)
...
loadVisualTab = function()
    ...
    createToggleUniversal(ScrollingFrame, "Тёмный туман", UDim2.new(0, 204, 0, 148), "WeatherWhiteFogEnabled", function(en)
        if en then
            _G.WeatherNightEnabled = false; _G.WeatherSunsetEnabled = false; _G.WeatherStormEnabled = false
            _G.WeatherStarsFogEnabled = false; _G.UltraGraphicsEnabled = false
        end
        updateWeather()
    end)

    createToggleUniversal(ScrollingFrame, "Ультра Графика", UDim2.new(0, 6, 0, 196), "UltraGraphicsEnabled", function(en)
        if en then
            _G.WeatherNightEnabled = false; _G.WeatherSunsetEnabled = false; _G.WeatherStormEnabled = false
            _G.WeatherStarsFogEnabled = false; _G.WeatherWhiteFogEnabled = false
        end
        updateWeather()
    end)
    ...
    espTitle.Position = UDim2.new(0, 6, 0, 248)
    espTitle.BackgroundTransparency = 1
    espTitle.Text = "ИГРОКИ И ESP"
    espTitle.TextColor3 = Color3.fromRGB(150, 150, 160)
    espTitle.Font = Enum.Font.GothamBold
    espTitle.TextSize = 11
    espTitle.TextXAlignment = Enum.TextXAlignment.Left

    createToggleUniversal(ScrollingFrame, "3D ESP", UDim2.new(0, 6, 0, 270), "ESP3DEnabled")
    createToggleUniversal(ScrollingFrame, "Имена ESP", UDim2.new(0, 204, 0, 270), "ESPNamesEnabled")
    createToggleUniversal(ScrollingFrame, "Скрыть ники", UDim2.new(0, 6, 0, 318), "HideNamesEnabled")

    ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 370)
end

tabPersonage.MouseButton1Click:Connect(function()
    playClickAnimation(tabPersonage)
    setTabActive(tabPersonage)
    loadPersonageTab()
end)

tabVisual.MouseButton1Click:Connect(function()
    playClickAnimation(tabVisual)
    setTabActive(tabVisual)
    loadVisualTab()
end)

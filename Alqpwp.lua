--[===================================================================]--
--  ROBLOX CHEAT: SHASIK CHEATS v9.42 - FULL JETPACK INTEGRATED
--[===================================================================]--
...
local totalTimerDuration = 5 * 60 -- Изменено на 5 минут (300 секунд)
local timerActive = false
local timeExpiredAlertShown = false

RunService.Heartbeat:Connect(function()
    if not isAuthorized or not timerActive then return end
    ...
    if remaining <= 0 then
        ...
        if not timeExpiredAlertShown then
            timeExpiredAlertShown = true
            timerActive = false
            
            -- Скрываем главное меню, если оно открыто
            ...
            -- Показываем уведомление с требованием нового ключа
            ShowAlert("Время истекло", "Таймер прошёл, введите новый ключ.", false, function()
                TextBoxBg.Text = ""
                OpenAuthMenu()
            end)
        end
...
LoginBtn.MouseButton1Click:Connect(function()
    ...
    -- Проверка на использование старого ключа после его истечения
    if timeExpiredAlertShown and (enteredKey == "Shasikcheats" or enteredKey == "Shasik" or enteredKey == "UserKey15m") then
        ShowAlert("Неверный ключ", "Срок действия этого ключа истек! Введите новый ключ.", false)
        return
    end
    ...
    if enteredKey == "Shasikcheats" or enteredKey == "Shasik" or enteredKey == "UserKey15m" then
        isAuthorized = true
        scriptStartTime = os.time()
        timerActive = true
        timeExpiredAlertShown = false
...
local function applyWhiteFog()
    ...
    atmosphere.Decay = Color3.fromRGB(180, 185, 195)
end
...
local function updateWeather()
    ...
    elseif _G.WeatherStormEnabled then
        ...
        Lighting.OutdoorAmbient = Color3.fromRGB(45, 55, 70)
        Lighting.Ambient = Color3.fromRGB(35, 45, 60)
        Lighting.FogStart = 0
        Lighting.FogEnd = 350
        Lighting.FogColor = Color3.fromRGB(45, 55, 70)
        local atmosphere = Lighting:FindFirstChildOfClass("Atmosphere") or Instance.new("Atmosphere", Lighting)
        atmosphere.Name = "CustomFogAtmosphere"
        atmosphere.Density = 0.55
        atmosphere.Color = Color3.fromRGB(50, 60, 75)
        atmosphere.Decay = Color3.fromRGB(30, 35, 45)

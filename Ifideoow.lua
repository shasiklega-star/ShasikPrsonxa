--[===================================================================]--
--  ROBLOX CHEAT: SHASIK CHEATS v9.41 - FULL JETPACK INTEGRATED
--[===================================================================]--
...
local MAX_UP_SPEED = 24
local MAX_DOWN_SPEED = -18
local SLOW_DRIFT_SPEED = -1.0
...
local function setupCharacterJetpack(char)
    ...
    local function createTankAndFire(offset, index)
        createCylinder("TankBody_"..index, 0.32, 1.5, tankPrimaryColor, Vector3.new(offset, -0.45, 0.62), CFrame.Angles(math.rad(90), 0, 0))
        createSphere("TankCap_"..index, 0.32, tankPrimaryColor, Vector3.new(offset, 0.3, 0.62))
        createCylinder("TankStripe_"..index, 0.33, 0.4, tankAccentColor, Vector3.new(offset, -0.3, 0.62), CFrame.Angles(math.rad(90), 0, 0))
        createCylinder("ClampTop_"..index, 0.34, 0.08, chromeColor, Vector3.new(offset, 0.05, 0.62), CFrame.Angles(math.rad(90), 0, 0))
        createCylinder("ClampBottom_"..index, 0.34, 0.08, chromeColor, Vector3.new(offset, -0.95, 0.62), CFrame.Angles(math.rad(90), 0, 0))
        createCylinder("Nozzle_"..index, 0.22, 0.3, chromeColor, Vector3.new(offset, -1.25, 0.62), CFrame.Angles(math.rad(90), 0, 0))
        createCylinder("NozzleInner_"..index, 0.15, 0.32, darkMetalColor, Vector3.new(offset, -1.26, 0.62), CFrame.Angles(math.rad(90), 0, 0))
        createCylinder("WireMain_"..index, 0.04, 0.35, wireColor, Vector3.new(offset * 0.6, -0.15, 0.58), CFrame.Angles(0, 0, math.rad(90)))
        createCylinder("WireLoop_"..index, 0.03, 0.35, darkMetalColor, Vector3.new(offset * 0.7, -0.65, 0.58), CFrame.Angles(math.rad(45), 0, 0))
        ...
        weld.Part0 = torso
        weld.Part1 = firePart
        weld.C0 = CFrame.new(offset, -1.38, 0.62) * CFrame.Angles(math.rad(180), 0, 0)
    ...
    createCylinder("BridgeTop", 0.06, 0.84, whiteColor, Vector3.new(0, -0.1, 0.62), CFrame.Angles(0, math.rad(90), 0))
    createCylinder("BridgeBottom", 0.06, 0.84, whiteColor, Vector3.new(0, -0.6, 0.62), CFrame.Angles(0, math.rad(90), 0))
end
...
local function removeJetpack(char)
    ...
    if torso then
        if torso:FindFirstChild("JetpackVelocity") then torso.JetpackVelocity:Destroy() end
        if torso:FindFirstChild("JetpackCoreAttachment") then torso.JetpackCoreAttachment:Destroy() end
        if torso:FindFirstChild("JetpackFireSound") then 
            torso.JetpackFireSound:Stop()
            torso.JetpackFireSound:Destroy() 
        end
        if torso:FindFirstChild("JetpackWindSound") then 
            torso.JetpackWindSound:Stop()
            torso.JetpackWindSound:Destroy() 
        end
        if torso:FindFirstChild("JetpackIdleSound") then 
            torso.JetpackIdleSound:Stop()
            torso.JetpackIdleSound:Destroy() 
        end
    end
...
local function updateWeather()
    for _, obj in pairs(Lighting:GetChildren()) do
        if obj:IsA("Sky") or obj:IsA("Atmosphere") or obj:IsA("Clouds") or obj:IsA("SunRays") or obj:IsA("PostEffect") then
            obj:Destroy()
        end
    end
    ...
    if _G.WeatherWhiteFogEnabled then applyWhiteFog()
    ...
    if _G.JetpackEnabled and character then
        ...
        if bodyVelocity and bodyVelocity.Parent and humanoid then
            ...
            if not isInAir and not isFlyingUp then
                ...
                for _, fire in ipairs(fireParticles) do fire.Enabled = false end
            else
                ...
                if isFlyingUp then
                    ...
                    if windSound and windSound.IsPlaying then windSound:Stop() end
                    if idleSound and idleSound.IsPlaying then idleSound:Stop() end
                    for _, fire in ipairs(fireParticles) do fire.Enabled = true end
                else
                    if fireSound and fireSound.IsPlaying then fireSound:Stop() end
...
loadPersonageTab = function()
    ...
    createToggleUniversal(ScrollingFrame, "Выдать джетпак", UDim2.new(0, LeftX, 0, row2Y), "JetpackEnabled", function(en)
        if en then
    ...
    createActionButton(ScrollingFrame, "Slap вверх", UDim2.new(0, RightX, 0, row5Y), function()
        ...
        if hrp then
            ...
            hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 90, hrp.AssemblyLinearVelocity.Z)
        end
    end)

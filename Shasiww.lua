...
loadPersonageTab = function()
    ...
    createToggleUniversal(ScrollingFrame, "Джетпак (Полет)", UDim2.new(0, LeftX, 0, row2Y), "JetpackEnabled", function(en)
        if en then
...
loadVisualTab = function()
    ...
    createToggleUniversal(ScrollingFrame, "FOV 1 (90)", UDim2.new(0, 6, 0, 26), "FOVEnabled", function(en)
        if en then _G.FOV2Enabled = false; _G.FOV3Enabled = false end
        updateFOV()
    end)

    createToggleUniversal(ScrollingFrame, "FOV 2 (105)", UDim2.new(0, 204, 0, 26), "FOV2Enabled", function(en)
        if en then _G.FOVEnabled = false; _G.FOV3Enabled = false end
        updateFOV()
    end)

    createToggleUniversal(ScrollingFrame, "FOV 3 (120)", UDim2.new(0, 6, 0, 74), "FOV3Enabled", function(en)
...
for i, name in pairs(tabs) do
    ...
    local function activateTab()
        ...
        if currentActive then
            ...
            currentActive.Button.TextColor3 = Color3.fromRGB(180, 180, 190)
            currentActive.Dot.Visible = false
        end

        tabContainer.BackgroundColor3 = Color3.fromRGB(24, 26, 36)
        stroke.Color = Color3.fromRGB(53, 132, 228)
